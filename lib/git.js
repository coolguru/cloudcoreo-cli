...
module.exports.init = function(dirPath, callback) {
    helper.git_cmd(dirPath, "init", {}, ['.'], callback);
};

module.exports.add_submodule = function(dir, git_url, module_path, callback){
    console.log(dir + ',' + git_url + ',' + module_path);
    helper.git_cmd(dir, "submodule add", {}, ['--force', git_url, module_path], function(err, data){
	if (err) { 
	    console.log(err);
	    callback(err, false);
	} else {
	    helper.git_cmd(dir, "submodule update",{},["--recursive", "--init"],function(err, stuff){
		if(err) {
		    console.log(err);
		} else {
		    callback(null, true);
...
module.exports.clone = function(git_url, dirPath, callback) {
	...
	helper.git_cmd(dirPath, "submodule update",{},["--recursive", "--init"],function(err, stuff){
