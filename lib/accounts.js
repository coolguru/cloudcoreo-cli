...
function readConfigFileSync(configFile, configs){
    ...
    for (var i = 0; i < sections.length; i++) {
        var sectionIt = sections[i];
        var key = sectionIt.name;
        if (sectionIt.name && sectionIt.accessKeyId) {
            key = sectionIt.name + ":" + sectionIt.accessKeyId;
        }
        configs[key] = sectionIt;
    }
}
...
function findAWSCredentialsSync() {
    ...
    var configfiles = [];
    if ( fs.existsSync(aws_config_dir)){
        configfiles=fs.readdirSync(aws_config_dir);
    }
    configfiles = configfiles.map(function(fname) {
        return path.join(aws_config_dir, fname);
    });
    if ( aws_config_file ) {
    ...
    var configs = {};
    for (var f = 0; f < configfiles.length; f++) {
        readConfigFileSync(configfiles[f], configs);
        var sections = Object.keys(configs);
        sections.forEach(function(key) {
            var section = configs[key];
            // Only keep complete sections
            if (section.name && section.accessKeyId) {
                // lets dedup this stuff
                var isDup = false;
                for (var j = 0; j < configurations.length; j++) {
