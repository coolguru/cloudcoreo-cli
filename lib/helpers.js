...
module.exports.generateReadme = function(){
                ...
                for ( var m_var in my_doc['variables'] ){
                    ...
                    for(var chk_var in combinedDictionary){
                        for(var x = 0; x < namespace_array.length; x++){
                            ...
                            if(combinedDictionary[chk_var]['namespace'].toUpperCase() == chk_namespace.toUpperCase()){
                                if(combinedDictionary[chk_var]['overrides']){
                                    ...
                                    for(var j = 0; j < combinedDictionary[chk_var]['overrides'].length; j++){
                                        ...
                                        if(override_var.split('::').slice(-1)[0].toUpperCase() == m_var.toUpperCase()){
                                            isOverridden = true;
                                        }
                                    }
                                }
                        ...
                        // namespace_array is the namespace of the variable we are checking - lets call it "us"/"we"
                        //    if "we" are in the ROOT namespace, we can override anything. i.e ROOT::PAUL::ALLEN::STUFF because it
                        //       is still in our tree.
                        //    if "we" are in the ROOT::FOO::BAR, we con only override things from ROOT::FOO::BAR and onward.
                        //       i.e if a variable is in ROOT::FOO we cannot override it.
                    ...
                    if ( isOverridden == false ) {
                        // we always want to store new because we are traversing
                        // from deepest dir up through the current working dir
                        // which means the last always get priority
